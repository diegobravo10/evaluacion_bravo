...
server.listen(3000, () => {
  console.log('Nodo B - WebSocket server listening on port 3000');
});
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', async (message) => {
    try {
      ...
      console.log('📨 Nodo B recibido:', data);
      ...
      trail.push('B_processed');

      // Simple business rule: if even, multiply by 2; if odd, keep same
      const isEven = (num % 2) === 0;
      const processed = isEven ? num * 2 : num * 1;
